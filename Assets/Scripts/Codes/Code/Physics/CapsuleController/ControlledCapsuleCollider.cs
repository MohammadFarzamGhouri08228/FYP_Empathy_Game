...
//This will collide against geometry and take appropriate action
    ...
    void TryMove(Vector3 a_Velocity, ref Vector2 o_PureVel)
    {
        for (int iteration = 0; iteration < 15; iteration ++)
        {
            ...
            List<RaycastHit2D> hits = CCState.CorrectCapsuleCastAll(GetDownCenter() + margin, GetUpCenter() + margin, m_Radius, direction, distanceToTravel, m_LayerMask);
    ...
    {
        List<RaycastHit2D> raycastHits = new List<RaycastHit2D>(Physics2D.CircleCastAll(m_CapsuleTransform.GetPosition(), m_Radius - m_GroundedMargin, -m_CapsuleTransform.GetUpDirection(), (m_CapsuleTransform.GetLength() * 0.5f) + m_GroundedCheckDistance + m_GroundedMargin, m_LayerMask));
        m_State.UpdateGroundedInfo(raycastHits);
        List<RaycastHit2D> rightCastResults = CCState.CorrectCapsuleCastAll(GetDownCenter(true), GetUpCenter(true), GetRadius() - m_SideCastMargin, m_CapsuleTransform.GetRightDirection(), m_SideCastDistance + m_SideCastMargin, m_LayerMask);
        List<RaycastHit2D> leftCastResults = CCState.CorrectCapsuleCastAll(GetDownCenter(true), GetUpCenter(true), GetRadius() - m_SideCastMargin, -m_CapsuleTransform.GetRightDirection(), m_SideCastDistance + m_SideCastMargin, m_LayerMask);
