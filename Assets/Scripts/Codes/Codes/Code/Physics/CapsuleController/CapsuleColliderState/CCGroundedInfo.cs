...
public class CCGroundedInfo : CGroundedInfo
{
    ...
    List<RaycastHit2D> m_ValidHits = new List<RaycastHit2D>();
    RaycastHit2D m_MostValidHit;
    ...
    public void UpdateWithCollisions(List<RaycastHit2D> a_Collisions)
    {
        m_ValidHits.Clear();
        m_IsGrounded = false;
        ...
        {
            float hitPointDot = Vector2.Dot(a_Collisions[i].point, Vector2.up);
            float downCenterDot = Vector2.Dot(downCenter, Vector2.up);
            float reqAngle = m_CapsuleCollider.GetMaxGroundedAngle();
            float angle = Vector2.Angle(Vector2.up, a_Collisions[i].normal);
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(m_CapsuleCollider.GetCapsuleTransform().GetDownCenter(), Vector2.down, m_CapsuleCollider.GetRadius() + m_CapsuleCollider.GetGroundedMargin(), m_CapsuleCollider.GetLayerMask());
