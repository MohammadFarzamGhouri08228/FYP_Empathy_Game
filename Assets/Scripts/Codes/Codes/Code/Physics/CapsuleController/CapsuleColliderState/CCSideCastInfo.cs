...
public class CCSideCastInfo : CSideCastInfo
{
    ...
    public void UpdateWithCollisions(List<RaycastHit2D> leftHitResults, List<RaycastHit2D> rightHitResults)
    {
        m_HasHitSide = false;
        m_WallCastCount = 0;
        ...
        List<RaycastHit2D> resultsToUse = null;
        ...
        RaycastHit2D correctingCast1, correctingCast2;
        Vector2 dirAlongNormal = CState.GetDirectionAlongNormal(Vector2.up, m_MostValidHit.normal);
        correctingCast1 = Physics2D.Raycast(m_MostValidHit.point + m_MostValidHit.normal * 0.02f + dirAlongNormal * 0.01f, -m_MostValidHit.normal, 0.03f, m_CapsuleCollider.GetLayerMask());
        correctingCast2 = Physics2D.Raycast(m_MostValidHit.point + m_MostValidHit.normal * 0.02f - dirAlongNormal * 0.01f, -m_MostValidHit.normal, 0.03f, m_CapsuleCollider.GetLayerMask());
        
        bool correctCast1Hit = correctingCast1.collider != null;
        bool correctCast2Hit = correctingCast2.collider != null;
        
        if (correctCast1Hit || correctCast2Hit)
        {
            RaycastHit2D correctCastToUse = m_MostValidHit;
            if (correctCast1Hit && correctCast2Hit)
            {
                float dot1 = Vector2.Dot(m_MostValidHit.normal, correctingCast1.normal);
                float dot2 = Vector2.Dot(m_MostValidHit.normal, correctingCast2.normal);
            ...
            {
                if (Vector2.Dot(m_MostValidHit.normal, correctingCast1.normal) < 0.95f)
                {
                    correctCastToUse = correctingCast1;
                }
            }
            ...
            {
                if (Vector2.Dot(m_MostValidHit.normal, correctingCast2.normal) < 0.95f)
                {
                    correctCastToUse = correctingCast2;
                }
            }
    ...
    void WallCast(RaycastHit2D a_SidecastHit)
    {
        if (Vector2.Angle(Vector2.up, a_SidecastHit.normal) > m_CapsuleCollider.GetMaxWallAngle() || Vector2.Angle(Vector2.up, a_SidecastHit.normal) < m_CapsuleCollider.GetMaxGroundedAngle())
        {
            m_WallCastCount = 0;
            return;
        }
        ...
        RaycastHit2D newHit;
        m_WallCastCount = 0;
        newHit = Physics2D.Raycast(startHitPos1, -normal, wallCastDistance + wallCastMargin, m_CapsuleCollider.GetLayerMask());
        if (newHit.collider != null)
        {
            m_WallCastCount++;
        }
        newHit = Physics2D.Raycast(startHitPos2, -normal, wallCastDistance + wallCastMargin, m_CapsuleCollider.GetLayerMask());
        if (newHit.collider != null)
        {
            m_WallCastCount++;
        }
        newHit = Physics2D.Raycast(startHitPos3, -normal, wallCastDistance + wallCastMargin, m_CapsuleCollider.GetLayerMask());
        if (newHit.collider != null)
        {
            m_WallCastCount++;
        }
